...
function addUser() {
    ...
    var newPassword = md5(document.getElementById("passwordinput").value);
     ...
     // stops code if no username or password is added
    if (newUsername == "" || document.getElementById("passwordinput").value == "" || newFname == "" || newLname == "") {
    console.log("account not added");
    document.getElementById("userAddResult").innerHTML = "One or more fields are empty";
    document.getElementById("userAddResult").classList.add('fail-whale');
    return;
    }
    ...
    try {
        xhr.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("userAddResult").classList.remove('fail-whale');
